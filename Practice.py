...
# Подключение к базе данных PostgresSQLdfsfsdf
def connect_to_db():
    try:
...
class AddRecordDialog(tk.Toplevel):
    ...
    def add(self):
        self.result = [entry.get() for entry in self.entries]
        self.destroy()
# Диалоговое окно для поиска по ФИО преподавателя# Диалоговое окно для поиска по ФИО преподавателя
# Диалоговое окно для поиска по ФИО преподавателяавыавыа
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("База данных PostgreSQL")
        ...
        self.edit_button = tk.Button(self, text="Редактирование запись", command=self.edit_record)
